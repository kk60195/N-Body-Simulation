...
for( i = 0 ; i < len ; i++){
	...
	Bodies[i + vx] += Bodies[i + fx] / Bodies[i + m]; //set vx += fx /m
	Bodies[i + vy] += Bodies[i + fy] / Bodies[i + m]; //set vy += fy /m

	Bodies[i + x] -= Bodies[i + vx]; //new x +=vx
	Bodies[i + y] -= Bodies[i + vy]; //new y +=vy
}
